    ...
    <AppShell>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative">
              ...
              <Card className="dossier-card rounded-3xl bg-card border border-border/40 overflow-hidden shadow-sm hover:shadow-elegant transition-all">
                <CardContent className="p-8">
                  <div className="flex flex-col xl:flex-row gap-8">
                    <div className="space-y-6 flex-1 border-b xl:border-b-0 xl:border-r border-border/40 pb-8 xl:pb-0 xl:pr-8">
                      <div className="space-y-3">
                        ...
                        <h1 className="font-display text-3xl font-bold tracking-tight">{order.title}</h1>
                      </div>
                    ...
                    {/* Right: Actions */}
                    <div className="xl:w-64 shrink-0 space-y-6 flex flex-col items-start xl:items-stretch text-left">
          ...
          {/* Right Column: Chat */}
          <div className="lg:col-span-5 xl:col-span-4 lg:sticky lg:top-24 h-[600px] lg:h-[calc(100vh-8rem)] flex flex-col">
            <Card className="dossier-card rounded-3xl border border-border/40 overflow-hidden shadow-sm hover:shadow-elegant transition-all flex flex-col bg-card h-full">
              <CardHeader className="px-6 py-5 border-b border-border/40 bg-muted/5 shrink-0">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
              ...
              <CardContent className="p-0 flex flex-col flex-1 overflow-hidden">
                <div className="chat-scroll flex flex-col gap-6 p-6 flex-1 overflow-y-auto min-h-0">
                  {messages.length === 0 && (
                    <div className="flex-1 flex flex-col items-center justify-center text-center text-muted-foreground gap-4 py-8">
                      <div className="h-14 w-14 rounded-full bg-muted/30 flex items-center justify-center">
                        <Send className="h-6 w-6 text-muted-foreground/20" />
                      </div>
                      <div className="text-center">
                        <p className="font-bold uppercase tracking-widest text-[10px] text-muted-foreground/40 mb-1">No messages</p>
                  ...
                  {messages.map((m, idx) => {
                    ...
                    const getRoundedClasses = () => {
                      if (isOwn) {
                        if (isFirstInGroup && isLastInGroup) return "rounded-[1.25rem]";
                        if (isFirstInGroup) return "rounded-[1.25rem] rounded-br-sm";
                        if (isLastInGroup) return "rounded-[1.25rem] rounded-tr-sm";
                        return "rounded-[1.25rem] rounded-r-sm";
                      } else {
                        if (isFirstInGroup && isLastInGroup) return "rounded-[1.25rem]";
                        if (isFirstInGroup) return "rounded-[1.25rem] rounded-bl-sm";
                        if (isLastInGroup) return "rounded-[1.25rem] rounded-tl-sm";
                        return "rounded-[1.25rem] rounded-l-sm";
                      }
                    };

                    return (
                      <div key={m.id} className={`flex group/msg ${isOwn ? "flex-row-reverse" : "flex-row"} gap-3 items-end ${!isFirstInGroup ? "-mt-4" : ""}`}>
                        <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-[0.8rem] font-display text-[9px] font-bold transition-all
                          ${isLastInGroup ? "opacity-100 scale-100 shadow-sm" : "opacity-0 scale-95 select-none pointer-events-none"}
                          ${isOwn ? "bg-primary text-primary-foreground" : "bg-card border border-border/40 text-muted-foreground"}`}>
                          {isOwn ? "ME" : "RP"}
                        </div>
                        <div className={`flex flex-col gap-1 max-w-[80%] ${isOwn ? "items-end" : "items-start"}`}>
                          <div className={`relative px-4 py-2.5 text-[13px] font-medium leading-relaxed shadow-sm transition-all hover:shadow-md
                            ${isOwn
                          ...
                          {isLastInGroup && (
                            <div className="flex items-center gap-1.5 px-2 font-display text-[9px] font-bold text-muted-foreground/40 uppercase tracking-widest opacity-0 group-hover/msg:opacity-100 transition-opacity">
                ...
                <AnimatePresence>
                  {violationWarning && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mx-6 mb-3 flex items-start gap-4 rounded-2xl border border-destructive/20 bg-destructive/5 p-4 text-xs text-destructive shadow-sm">
                      ...
                      <button onClick={() => setViolationWarning("")} className="h-6 w-6 flex items-center justify-center rounded-lg hover:bg-destructive/10 transition-colors"><X className="h-3 w-3" /></button>
                    </motion.div>
                  )}
                </AnimatePresence>

                <div className="p-4 border-t border-border/40 bg-card shrink-0">
                  <div className="relative group/input flex items-end gap-2 rounded-[1.5rem] bg-muted/30 border border-border/40 p-1.5 pl-4 focus-within:border-primary/40 transition-all focus-within:bg-card focus-within:shadow-sm">
                    <Textarea
                      placeholder="Type a message..."
                      value={msgContent}
                      onChange={(e) => setMsgContent(e.target.value)}
                      rows={1}
                      className="flex-1 bg-transparent border-0 ring-0 focus-visible:ring-0 focus-visible:ring-offset-0 min-h-[40px] py-2.5 text-[13px] font-medium resize-none placeholder:text-muted-foreground/40"
                      onKeyDown={(e) => {
                    ...
                    <Button
                      ...
                      className="shrink-0 h-10 w-10 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground transition-transform shadow-sm p-0 disabled:opacity-50"
                    >
                      {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                    </Button>
                  </div>
                  <p className="font-display text-[9px] font-medium text-muted-foreground/40 text-center mt-3">
